...
2026-04-22: **resolve-vault.sh cron fallback** (claude-obsidian issue #31, PR #32). Added third fallback to `scripts/resolve-vault.sh`: reads `~/.claude/settings.local.json` via `jq` (primary) or `python3` (fallback) to resolve `pluginConfigs[*claude-obsidian*].options.vault_path`. Fixes `wiki-lint-cron.sh` silently failing out of the box when scheduled via cron. Pattern: use `contains("plugin-name")` key matching (not hardcoded install ID); fallback order is arg > CWD wiki/ > settings.local.json > error. See [[out-of-session-plugin-config-access]].

2026-04-22: **Subagents vs TeamCreate autoresearch complete** (input for misiekhardcore/claude-workflow issue #30). 10 pages filed. Verified Anthropic's ~7× token multiplier for TeamCreate from official /en/costs. Each teammate loads **full CLAUDE.md + MCP + skills + spawn prompt**; lead's conversation history does NOT carry. Primary decision pivot is **mid-task communication need** — subagents report back only; teammates talk via shared mailbox. Empirical basis: Princeton NLP single-agent matches MAS on **64%** of benchmarks at half cost; Google DeepMind's 180-config study — sequential tasks degrade up to **70%** under MAS. Parallelism buys wall-clock (4× on disjoint tasks), not tokens. Sweet spot: **3–5 teammates, 5–6 tasks each**. New rubric replaces "3+ independent files" heuristic with: (1) communication pivot, (2) file disjointness, (3) classifiably-parallel task shape, (4) expected wall-clock payoff ≥ 3× to justify 7× token premium. Headline pages: [[Research: Subagents vs TeamCreate Decision Rubric]], [[subagent-vs-teamcreate-rubric]], [[teamcreate-architecture]], [[subagent-spawn-mechanics]], [[agent-scaling-empirical-evidence]].
